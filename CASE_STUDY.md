...
The existing save features on social platforms are black holes. No search, no time-based filtering, no way to filter by ingredients on hand, no nutritional visibility. The recipes existed. The retrieval system didn't.
...
Auth was intentionally included in the MVP. Recipe data needs to persist across sessions and devices — a parent who spends time importing 50 recipes cannot afford to lose that data when the browser is cleared, or the device is switched. Supabase row-level security also ensures each user's recipe collection stays private. The friction cost of a login wall was a conscious tradeoff against data durability and privacy.
...
| Feature | Reason Cut |
|---|---|
| Meal planning calendar | Solves a different job (planning) vs. the core job (right-now retrieval). Premature before validating the core loop. |
| Social sharing & ratings | Community features require critical mass. No value at the MVP stage. |
...
- Is the login wall causing a meaningful drop-off before the first value is experienced?
...
Requiring a login before users experience any value is a known conversion killer. I made the tradeoff consciously for data durability, but it's the first thing I'd A/B test with sufficient traffic.
