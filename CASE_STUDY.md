...
FeatureReason CutMeal planning calendarSolves a different job (planning ahead) vs. the core job (right-now retrieval). Premature before validating the core loop.Social sharing & ratingsCommunity features require critical mass. No value at MVP stage.Nutritional scoringComplex to implement accurately. Risk of false precision building distrust with parents.Direct Instagram API integrationPlatform policy complexity and rate limiting create dependency risk. Solved at the URL scraping level instead — works across all platforms without the fragility.Saved pantry staplesHigh-value feature but adds setup friction. Deferred to v2 — users must re-tap common ingredients each session for now.
